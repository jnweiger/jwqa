...
#  - https://github.com/owncloud/ocis/tree/master/deployments/examples/oc10_ocis_parallel/config/ldap/ldif
...
##  generate_users_and_group lemming  3 3 lem3  >> $ldif/45_lem100.ldif
   ...
   generate_users_and_group rabbit  1 100 rabbit >  $ldif/45_rab100.ldif
fi
...
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/update-mdb-acl.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/users-ou.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/readonly-user.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/usera.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/jwqa-ou.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/memberof.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/memberof_part2.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/add_refint.ldif
# ldapadd -Y EXTERNAL -H ldapi:/// -f init/openldap/add_to_group.ldif
# ldapsearch -x -b "dc=jwqa,dc=org" -H ldapi:/// -D "cn=readonly,ou=people,dc=owncloud,dc=company" -W "(&(objectClass=inetOrgPerson)(uid=usera))"
#
# slapcat | less
#
# # show the config
