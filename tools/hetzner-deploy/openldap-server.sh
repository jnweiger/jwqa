...
generate_ldif=true	# must be true or false, use false, after you hand-edited the files in $ldif .
...
$generate_ldif && cat <<EOF1 > $ldif/10_owncloud_schema.ldif
...
## the dc=owncloud,dc=com is commented out here, to avoid complains that it already exitst.
...
$generate_ldif && generate_users_and_group lemming 1000  > $ldif/45_lem1000.ldif
$generate_ldif && generate_users_and_group rabbit  1000 >> $ldif/45_lem1000.ldif
...
ldapsearch -x -H ldap://$ldapserver:$ldapport -b dc=owncloud,dc=com -D "$admin_dn" -w "$admin_pass" -v
...
Finally from the login shell:
 ...
 ldapsearch -x -H ldap://$ldapserver:$ldapport -b dc=owncloud,dc=com -D "$admin_dn" -w "$admin_pass" '(uid=lemming0123)'
