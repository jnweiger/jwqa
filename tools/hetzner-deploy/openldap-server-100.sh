...
generate_ldif=false	# must be true or false
...
dn: ou=users,dc=jwqa,dc=example,dc=com
...
dn: uid=marie,ou=users,dc=example,dc=com
objectClass: inetOrgPerson
objectClass: organizationalPerson
objectClass: person
objectClass: posixAccount
...
dn: uid=richard,ou=users,dc=example,dc=com
objectClass: inetOrgPerson
objectClass: organizationalPerson
objectClass: person
objectClass: posixAccount
...
dn: uid=jeff,ou=users,dc=example,dc=com
objectClass: inetOrgPerson
objectClass: organizationalPerson
objectClass: person
objectClass: posixAccount
...
dn: ou=groups,dc=jwqa,dc=example,dc=com
objectClass: organizationalUnit
ou: groups
...
dn: cn=physics-lovers,ou=groups,dc=example,dc=com
objectClass: top
...
gidNumber: 30007
uniqueMember: uid=einstein,ou=users,dc=example,dc=com
uniqueMember: uid=marie,ou=users,dc=example,dc=com
uniqueMember: uid=richard,ou=users,dc=example,dc=com
...
uniqueMember: uid=marie,ou=users,dc=example,dc=com
uniqueMember: uid=richard,ou=users,dc=example,dc=com
uniqueMember: uid=jeff,ou=users,dc=example,dc=com
uniqueMember: uid=admin,ou=users,dc=example,dc=com

dn: cn=hackers,ou=groups,dc=example,dc=com
# objectClass: groupOfUniqueNames
objectClass: extensibleObject
objectClass: posixGroup
objectClass: top
...
dn: cn=sailors,ou=groups,dc=example,dc=com
# objectClass: groupOfUniqueNames
objectClass: extensibleObject
objectClass: posixGroup
objectClass: top
...
gidNumber: 30001
uniqueMember: uid=einstein,ou=users,dc=example,dc=com
uniqueMember: cn=hackers,ou=groups,dc=example,dc=com
EOF3
...
# Start dn with uid (user identifier / login), not cn (Firstname + Surname)
dn: uid=$namepre$namecnt,ou=users,dc=example,dc=com
objectClass: inetOrgPerson
objectClass: organizationalPerson
objectClass: person
objectClass: posixAccount
...
dn: cn=${namepre}s,ou=groups,dc=example,dc=com
objectClass: top
objectClass: extensibleObject
objectClass: posixGroup
# objectClass: groupOfUniqueNames
...
EOG
  ...
  for g in $(seq -w $cnt_from $cnt_to); do
    echo "uniqueMember: uid=$namepre$g,ou=users,dc=example,dc=com"
  done
  echo ""
}

if $generate_ldif; then
  generate_users_and_group lemming  0 009 lem0  >  $ldif/45-lem100.ldif
  generate_users_and_group lemming 10 019 lem1  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 20 029 lem2  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 30 039 lem3  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 40 049 lem4  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 50 059 lem5  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 60 069 lem6  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 70 079 lem7  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 80 089 lem8  >> $ldif/45-lem100.ldif
  generate_users_and_group lemming 90 099 lem9  >> $ldif/45-lem100.ldif
  generate_users_and_group rabbit  1 100 rabbit >> $ldif/45-lem100.ldif
fi
...
opts="-v $mount $ports --env LDAP_ADMIN_PASSWORD=$admin_pass --env LDAP_ORGANISATION=jwqa --env LDAP_DOMAIN=jwqa.example.com"
...
ldapsearch -x -H ldap://$ldapserver:$ldapport -b dc=example,dc=com -D "$admin_dn" -w "$admin_pass" -v
...
-----------------------------------------------
 ...
 ldapsearch -x -H ldap://$ldapserver:$ldapport -b dc=jwqa,dc=example,dc=com -D "$admin_dn" -w "$admin_pass" '(uid=lemming0123)'
